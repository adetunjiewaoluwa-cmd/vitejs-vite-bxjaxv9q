...
/* ─────────────────────────────
   SUPABASE (SINGLE FILE SETUP)
───────────────────────────── */
const SUPABASE_URL = "https://zafvajcnwyzjumyklyni.supabase.co";
const SUPABASE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InphZnZhamNud3l6anVteWtseW5pIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcyMDkxMTEsImV4cCI6MjA5Mjc4NTExMX0.CIRzRcbf95yGerwDz6aiLz-s2fXXDnMfhfMaETm0A3s";

const db = {
  async getAll() {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/profiles?select=*&order=joined_at.desc`,
      {
        headers: {
          apikey: SUPABASE_KEY,
...
const COMPANY_EMAIL = "thevoryel@gmail.com";
...
export default function App() {
  const [role, setRole] = useState<"client" | "freelancer" | "both" | "">("");
  ...
  const handleSubmit = async () => {
    ...
    if (!acceptedTerms) return alert("You must accept terms (10% commission)");
    ...
    <div style={{ padding: 20 }}>
      ...
      <div>
        <label>
          ...
          />
          I agree to Terms & Conditions (10% commission applies)
        </label>
      </div>
      ...
      {/* ─────────────────────────────────────────────
          🔴 YOUR ORIGINAL 749 LINES GO BELOW
          (DO NOT MODIFY THEM)
      ───────────────────────────────────────────── */}
  ...
  return (
    <div style={{background:T.bg,color:T.white,minHeight:"100vh",fontFamily:"'Cinzel',serif"}}>
      ...
      {page==="signup"&&<Auth mode="signup" setPage={setPage} onAuth={onAuth} members={members}/>}
                                                     ...
                                                     }====== PASTE ORIGINAL APP.TSX CODE HERE ====== */}
